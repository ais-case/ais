      ...
      def self.fromPayload(payload)
        ...
        if msg_type == 1 or msg_type == 2 or msg_type == 3
          ...
          if decoded.length >= 424
            message = Message5.new(Datatypes::Int.from_bit_string(decoded[8..37]).value)
            message.vessel_type = Domain::VesselType::new(Datatypes::Int.from_bit_string(decoded[232..239]).value)
            message
          else
            nil
          end
